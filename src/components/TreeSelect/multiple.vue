<template>
  ...
  >
    ...
    <el-option :value="valueTitle"  class="options">
      <el-tree
        id="tree-option"
        ref="selectTree"
        :accordion="accordion"
...
export default {
  ...
  methods: {
    ...
    initHandle() {
      if (this.valueId) {
        var arrNew = []
        for (var i in this.valueId) {
          arrNew.push(
            this.$refs.selectTree.getNode(this.valueId[i]).data[
              this.props.label
            ]
          )
        }
        this.valueTitle = [].concat(Array.from(new Set(arrNew)))
        // 初始化显示
    ...
    handleNodeClick(node) {
      ...
      if (conditionOne || this.props.identifier.length == '0') {
        ...
        this.valueId = Array.from(new Set(this.valueId))
