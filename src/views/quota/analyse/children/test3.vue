    ...
    <div class="content">
      ...
      <div class="tableContent">
        <el-collapse v-model="activeNames">
          <el-collapse-item title="最大十家客户授信集中度" name="1" style="margin-top:0px">
...
<script>
  ...
  import { queryCreditTenCusRpt, downCreditTenCusRpt } from '@/api/rcd/limitAnalyse/limitAnalyse'
  ...
  export default {
    ...
    methods: {
      query() {
        ...
        queryCreditTenCusRpt(this.selectionOrgId, this.tableDate.year, this.tableDate.month, this.ruleForm.moneyUnit).then(res => {
          const datas = res.data.data
          this.allDatas = datas.creditTenCusRptVOList
          this.formData = datas.quotaInfoVO
          this.tableContent = datas.creditTenCusRptVOList
          if (datas.creditTenCusRptVOList.length > 0) {
            this.tableDate.month = datas.creditTenCusRptVOList[0].totalMonth
            this.tableDate.year = datas.creditTenCusRptVOList[0].totalYear
          }

          // 图状数据展示待开发
          this.option1.series[0].data = this.option2.series[0].data = this.option3.series[0].data = datas.creditTenCusRptVOList.map(item => {
            return item.crdBalance
          })
          this.option1.series[1].data = this.option2.series[1].data = this.option3.series[1].data = datas.creditTenCusRptVOList.map(item => {
            return item.change
          })
          this.option1.xAxis[0].data = this.option2.xAxis[0].data = this.option3.xAxis[0].data = datas.creditTenCusRptVOList.map(item => {
      ...
      dowmloadExcel() {
        ...
        downCreditTenCusRpt(this.selectionOrgId, year, month, this.ruleForm.moneyUnit).then(res => {
          const content = res
