    ...
    <div class="content">
      ...
      <div class="tableContent">
        <el-collapse v-model="activeNames">
          <el-collapse-item title="最大十家集团客户授信集中度" name="1" style="margin-top:0px">
          ...
          <el-collapse-item title="集团客户明细表" name="2" style="margin-top:0px">
            <!-- 表格部分 -->
            <el-table :data="tableContent" border style="width:100%;margin-bottom:-25px;margin-top:-5px"  max-height="180px" size="mini">
              ...
              <el-table-column prop="crdBalance" label="贷款余额">
                <template slot-scope="scope">{{ scope.row.crdBalance | NumsFormat }}</template>
              </el-table-column>
...
<script>
  ...
  import { queryCreditTenGroupCusRpt, downCreditTenGroupCusRpt } from '@/api/rcd/limitAnalyse/limitAnalyse'
  ...
  export default {
    data() {
      return {
        ...
        pm: '',
        // 表格详情部分 中间标题
        toptitle: '最大十家集团客户授信集中度及集团客户明细表',
        // 表格详情部分 左侧日期
        tableDate: {
          year: '',
          month: ''
    ...
    methods: {
      query() {
        ...
        queryCreditTenGroupCusRpt(this.selectionOrgId, this.tableDate.year, this.tableDate.month, this.ruleForm.moneyUnit).then(res => {
          const datas = res.data.data
          this.allDatas = datas.creditTenGroupCusRptVOList
          this.formData = datas.quotaInfoVO
          this.tableContent = datas.creditTenGroupCusRptVOList
          if (datas.creditTenGroupCusRptVOList.length > 0) {
            this.tableDate.month = datas.creditTenGroupCusRptVOList[0].totalMonth
            this.tableDate.year = datas.creditTenGroupCusRptVOList[0].totalYear
          }

          // 图状数据展示待开发
          this.option1.series[0].data = this.option2.series[0].data = this.option3.series[0].data = datas.creditTenGroupCusRptVOList.map(item => {
            return item.crdBalance
          })
          this.option1.series[1].data = this.option2.series[1].data = this.option3.series[1].data = datas.creditTenGroupCusRptVOList.map(item => {
            return item.change
          })
          this.option1.xAxis[0].data = this.option2.xAxis[0].data = this.option3.xAxis[0].data = datas.creditTenGroupCusRptVOList.map(item => {
      ...
      dowmloadExcel() {
        ...
        downCreditTenGroupCusRpt(this.selectionOrgId, year, month, this.ruleForm.moneyUnit).then(res => {
          const content = res
