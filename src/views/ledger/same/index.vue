...
<template>
  <div class="container">
    ...
    <div class="table">
      ...
      <template>
        <el-table :data="tableData" style="width: 100%" size="small" border stripe height="450" v-loading="loading">
          <el-table-column type="index" label="序号" width="50" align="center" fixed :resizable="false" />
          ...
          <!-- <el-table-column prop="userNum" label="客户经理" width="120" :resizable="false" align="center">
          ...
          <el-table-column prop="certNum" label="证件号码" :resizable="false" align="center" width="150" />
          <el-table-column prop="currencyCdName" label="币种" :resizable="false" align="center" width="190">
          ...
          <el-table-column prop="limitEarmark" label="总圈存额度" :resizable="false" align="center" width="110">
            <template slot-scope="scope">{{ scope.row.limitEarmark | NumsFormat }}</template>
          </el-table-column>
          <el-table-column prop="limitEarmarkUsed" label="总圈存已用额度" :resizable="false" align="center" width="120">
            <template slot-scope="scope">{{ scope.row.limitEarmarkUsed | NumsFormat }}</template>
          </el-table-column>
          <el-table-column prop="limitEarmarkAvi" label="圈存可用额度" :resizable="false" align="center" width="110">
            <template slot-scope="scope">{{ scope.row.limitEarmarkAvi | NumsFormat }}</template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="150" :resizable="false" align="center">
            <template slot-scope="scope">
              <el-button class="textBtn" type="text" size="small" icon="el-icon-view" @click="toMsg(scope.row)">查看</el-button>
    ...
    <!-- 机构选择dialog框 -->
    <el-dialog class="deptDialog" title="请选择机构" :visible.sync="deptDialogVisible" width="55%" height="420" @closed="closeDialogDept"
      :lock-scroll="false">
      <!-- 查询条件 -->
      <div class="search">
...
import { getList } from '@/api/rcd/standBook/same'
...
export default {
  data() {
    return {
      ...
      // =====导出功能需要传递的数据=====
      header: ['主办机构', '客户编号', '客户名称', '证件类型', '证件号码', '币种', '总授信额度', '总额度已用', '总额度可用', '总圈存额度', '总圈存已用额度', '圈存可用额度'], // 表头中文名称
      content: ['orgNumName', 'customerNum', 'customerName', 'certTypeName', 'certNum', 'currencyCdName', 'limitCredit', 'limitUsed', 'limitAvi', 'limitEarmark', 'limitEarmarkUsed', 'limitEarmarkAvi'], // 表头字段
      title: '同业客户额度台账表', // 导出的excel名称
      // =====表格区数据=====
      tableData: [],
      alltableData: [],//隐藏表格
      // 分页数据
  ...
  created() {
    this.ruleForm = JSON.parse(sessionStorage.getItem("sameFormData")) || {}
  },
  methods: {
    // 点击搜索按钮触发该事件
    searchInfo() {
      if (this.ruleForm.customerName == undefined && this.ruleForm.customerNum == undefined && this.ruleForm.certNum == undefined && this.ruleForm.orgNum == undefined) {
        ...
        sessionStorage.setItem('sameFormData', JSON.stringify(this.ruleForm))
        this.getTableData()
      }
    },
    // 获取table表格数据
    ...
    // 点击360视图按钮，触发该函数
    to360view(val) {
      this.$router.push({ name: 'sameView', params: { val: val } })
      sessionStorage.setItem('sameval', JSON.stringify(val))
    },
    // 点击表格内客户信息按钮，触发该函数
    customerInfo(val) {
    ...
    // 双击表格中某一行，跳转到详情页
    toMsg(val) {
      this.$router.push({ name: 'sameMsg', params: { val: val } })
      sessionStorage.setItem('sameMsgVal', JSON.stringify(val))
    },
    // 获取当前登录用户相关信息
    getInfoCommon() {
      getInfo().then(res => {
    ...
    inputChange(e) {
      const o = e.target;
      o.value = o.value.replace(/[^\w\.\/]/ig, ''); // 清除除了中文以外的输入的字符
      this.name = o.value;
    }
  },
  components: {
