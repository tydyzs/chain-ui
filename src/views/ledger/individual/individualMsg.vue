  ...
  <div class="corporateMsgCon">
    ...
    <el-card class="con">
      ...
      <el-table :data="limitDeaTableData" style="width: 100%" size="small" border stripe>
        ...
        <el-table-column prop="crdProductName" label="额度产品" align="center" :resizable="false" width="180"></el-table-column>
        <el-table-column prop="crdProductTypeName" label="额度类型" align="center" :resizable="false" width="110">
        ...
        <el-table-column prop="isCycleName" label="是否可循环" align="center" :resizable="false" width="100">
          <!-- <template slot-scope="scope">{{ isCycleDict[scope.row.isCycle] }}</template> -->
        </el-table-column>
        <el-table-column prop="limitCredit" label="授信额度" align="center" :resizable="false">
          <template slot-scope="scope">{{ scope.row.limitCredit | NumsFormat }}</template>
        </el-table-column>
    ...
    <el-card class="con">
      ...
      <!-- 分页部分 -->
      <el-pagination @size-change="handleSizeChange7" @current-change="handleCurrentChange7" :current-page="contractPage.pagenum"
    ...
    <el-card class="con">
      ...
      <el-table :data="borrowTableData" style="width: 100%" size="small" border stripe>
        ...
        <el-table-column prop="beginDate" label="借据起始日" align="center" :resizable="false" width="100"></el-table-column>
        <el-table-column prop="endDate" label="借据到期日" align="center" :resizable="false" width="100"></el-table-column>
    ...
    <el-card class="con">
      ...
      <el-table :data="assureTableData" style="width: 100%" size="small" border stripe>
        ...
        <el-table-column prop="subcontractNum" label="担保合同编号" align="center" :resizable="false" width="110"></el-table-column>
        <el-table-column prop="suretyNum" label="担保物编号" align="center" :resizable="false" width="100"></el-table-column>
        <el-table-column prop="suretyCustomerNum" label="抵质押人/保证人客户编号" align="center" :resizable="false" width="180"></el-table-column>
        <el-table-column prop="subcontractTypeName" label="担保合同类型" align="center" :resizable="false" width="110">
          <!-- <template slot-scope="scope">{{ subcontractTypeDict[scope.row.subcontractType] }}</template> -->
        </el-table-column>
        <el-table-column prop="pledgeTypeName" label="担保物类型" align="center" :resizable="false" width="110">
          <!-- <template slot-scope="scope">{{ pledgeTypeDict[scope.row.pledgeType] }}</template> -->
        </el-table-column>
        <el-table-column prop="currencyCdName" label="币种" align="center" :resizable="false" width="200">
          <!-- <template slot-scope="scope">{{ currencyCdDict[scope.row.currencyCd] }}</template> -->
        </el-table-column>
        <el-table-column prop="subcontractAmt" label="担保合同金额" align="center" :resizable="false" width="110">
          <template slot-scope="scope">{{ scope.row.subcontractAmt | NumsFormat }}</template>
        </el-table-column>
        <el-table-column prop="suretyAmt" label="本次担保金额" align="center" :resizable="false" width="110">
    ...
    <el-card class="con">
      <span>信用卡信息</span>
      <!-- 表格详情 -->
      <el-table :data="creditTableData" style="width: 100%" size="small" border stripe>
        <el-table-column prop="customerName" label="客户名称" align="center" :resizable="false" width="150"></el-table-column>
        <el-table-column prop="customerNum" label="客户编号" align="center" :resizable="false" width="80"></el-table-column>
        <el-table-column prop="acctNum" label="账号编号" align="center" :resizable="false" width="120"></el-table-column>
        <el-table-column prop="currencyCdName" label="币种" align="center" :resizable="false" width="200">
          <!-- <template slot-scope="scope">{{ currencyCdDict[scope.row.currencyCd] }}</template> -->
        </el-table-column>
        <el-table-column prop="creditLimit" label="信用额度" align="center" :resizable="false">
          <template slot-scope="scope">{{ scope.row.creditLimit | NumsFormat }}</template>
        </el-table-column>
        <el-table-column prop="tempLimit" label="临时额度" align="center" :resizable="false">
          <template slot-scope="scope">{{ scope.row.tempLimit | NumsFormat }}</template>
        </el-table-column>
        <el-table-column prop="currBal" label="当前余额" align="center" :resizable="false">
          <template slot-scope="scope">{{ scope.row.currBal | NumsFormat }}</template>
        </el-table-column>
        <el-table-column prop="billingCycle" label="账单周期" align="center" :resizable="false"></el-table-column>
        <el-table-column prop="tempLimitBeginDate" label="临时额度开始日期" align="center" :resizable="false" width="140"></el-table-column>
        <el-table-column prop="tempLimitEndDate" label="临时额度结束日期" align="center" :resizable="false" width="140"></el-table-column>
        <el-table-column prop="orgNumName" label="核算机构" align="center" :resizable="false" width="120">
          <!-- <template slot-scope="scope">{{ orgNumDict[scope.row.orgNum] }}</template> -->
        </el-table-column>
      </el-table>
      <!-- 分页部分 -->
      <el-pagination @size-change="handleSizeChange6" @current-change="handleCurrentChange6" :current-page="creditPage.pagenum"
        :page-sizes="[3, 5, 10, 20]" :page-size="creditPage.pagesize" layout="total, sizes, prev, pager, next, jumper"
        :total="creditPage.total"></el-pagination>
    </el-card>
  </div>
</template>
...
import {
  ...
  getApplySerialPage,
  getListGrtPage,
  getCrdCcInfoAndCustomerNamePage,
  getList
} from '@/api/rcd/standBook/individual'
...
export default {
  data() {
    return {
      ...
      steps: ['基本信息', '额度信息', '额度明细信息', '合同信息', '借据信息', '占用恢复信息', '对外担保信息', '信用卡信息']
    }
  },
  created() {
    // console.log(this.$route.params.val, '打印传递的值')
    // this.basicInfoForm = this.$route.params.val
    this.basicInfoForm = JSON.parse(sessionStorage.getItem("individualMsgVal"))
    // this.customerNum = this.$route.params.val.customerNum
    this.customerNum = this.basicInfoForm.customerNum
    //机构号赋值
    // this.orgNumForThisCustomer = this.$route.params.val.orgNumAnother
    this.orgNumForThisCustomer = this.basicInfoForm.orgNum
    // 下拉框获取当前客户授信机构
    this.getCrdOrgByCustCommon()
    // ========== 表单、表格填入数据 ==========
    this.getIndividualListCommon()
    ...
    this.getCrdCcInfoAndCustomerNamePageCommon()
    // 字典翻译
  ...
  methods: {
    ...
    getListCommon() {
      ...
      getList('1', '1', '', '', this.orgNumForThisCustomer, this.customerNum, '', '', '1').then(res => {
        console.log(res.data.data.records[0], "res.data.data.records[0]");
    ...
    //信用卡信息
    getCrdCcInfoAndCustomerNamePageCommon() {
      getCrdCcInfoAndCustomerNamePage(
        this.creditPage.pagenum,
        this.creditPage.pagesize,
        this.customerNum,
        // 'C002',
        this.orgNumForThisCustomer
        // '01000'
      ).then(res => {
        console.log(res, "信用卡信息");
        this.creditTableData = res.data.data.records;
        this.creditPage.total = res.data.data.total;
      });
    },
    // 字典翻译
    // dictTranslateCommon() {
    ...
    //   })
    //   // 额度类型
    //   dictTranslate('CD000211').then(res => {
    ...
    // 监听 信用卡信息 中分页 每页条数 改变的事件
    handleSizeChange6(newSize) {
      console.log(`每页 ${newSize} 条`)
      this.creditPage.pagesize = newSize
      // 调用方法，重新获取列表数据
      this.getCrdCcInfoAndCustomerNamePageCommon()
    },
    // 监听 信用卡信息 中分页 页码值 改变的事件
    handleCurrentChange6(newPage) {
      console.log(`当前页: ${newPage}`)
      this.creditPage.pagenum = newPage
      // 调用方法，重新获取列表数据
      this.getCrdCcInfoAndCustomerNamePageCommon()
    },
    // 电梯导航跳转功能
    ...
    onScroll() {
      ...
      console.log(scrolled, '000');
      this.steps.forEach((item, index) => {
        console.log(item, index, jump[index].offsetTop - 85);
