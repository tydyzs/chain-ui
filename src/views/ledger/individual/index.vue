...
<template>
  <div class="container">
    ...
    <div class="table">
      ...
      <template>
        <el-table :data="alltableData" style="width: 100%" size="small" border stripe max-height="400" v-if="flag=false">
          <el-table-column type="index" label="序号" align="center" width="50" fixed :resizable="false" />
          <el-table-column prop="orgNumName" label="主办机构" :resizable="false" align="center" width="180"></el-table-column>
          ...
          <el-table-column prop="certTypeName" label="证件类型" :resizable="false" align="center" width="180"></el-table-column>
          <el-table-column prop="certNum" label="证件号码" width="200" :resizable="false" align="center" />
          <el-table-column prop="currencyCdName" label="币种" :resizable="false" align="center" width="180"></el-table-column>
          <el-table-column prop="limitCredit" label="总授信额度" :resizable="false" align="center" width="110">
            <template slot-scope="scope">{{ scope.row.limitCredit | NumsFormat }}</template>
          </el-table-column>
          <el-table-column prop="limitUsed" label="总额度已用" :resizable="false" align="center" width="110">
    ...
    <!-- 机构选择dialog框 -->
    <el-dialog class="deptDialog" title="请选择机构" :visible.sync="deptDialogVisible" width="55%" height="420" @closed="closeDialogDept" :lock-scroll="false">
      <!-- 查询条件 -->
      <div class="search">
...
import { getList } from '@/api/rcd/standBook/individual'
...
export default {
  data() {
    return {
      ...
      // =====导出功能需要传递的数据=====
      header: ['主办机构', '客户编号', '客户名称', '证件类型', '证件号码', '币种', '总授信额度', '总额度已用', '总额度可用', '总授信敞口', '总敞口已用', '总敞口可用'], // 表头中文名称
      content: ['orgNumName', 'customerNum', 'customerName', 'certTypeName', 'certNum', 'currencyCdName', 'limitCredit', 'limitUsed', 'limitAvi', 'expCredit', 'expUsed', 'expAvi'], // 表头字段
      title: '个人客户额度台账表', // 导出的excel名称
      // =====表格区数据=====
      tableData: [],
      alltableData: [],//隐藏表格
      // 分页数据
  ...
  created() {
    this.ruleForm = JSON.parse(sessionStorage.getItem("individualFormData")) || {}
  },
  ...
  methods: {
    ...
    searchInfo() {
      if (this.ruleForm.customerName == undefined && this.ruleForm.customerNum == undefined && this.ruleForm.certNum == undefined && this.ruleForm.orgNum == undefined) {
        ...
        sessionStorage.setItem('individualFormData', JSON.stringify(this.ruleForm))
        this.getTableData()
      }
    },
    // 获取table表格数据
    ...
    // 点击360视图按钮，触发该函数
    to360view(val) {
      this.$router.push({ name: 'individualView', params: { val: val } })
      sessionStorage.setItem('individualval', JSON.stringify(val))
    },
    // 点击表格内客户信息按钮，触发该函数
    customerInfo(val) {
    ...
    // 双击表格中某一行，跳转到详情页
    toMsg(val) {
      this.$router.push({ name: 'individualMsg', params: { val: val } })
      sessionStorage.setItem('individualMsgVal', JSON.stringify(val))
    },
    // 获取当前登录用户相关信息
    getInfoCommon() {
      getInfo().then(res => {
